...
FIELD_HINTS = {
    ...
    "bodyWater": "体水分率",
    "intakeCalories": "その日に食べた分のカロリー。食事管理アプリ「あすけん」の『摂取カロリー』タブ専用",
    "calories": "その日に消費した分のカロリー。ヘルスケアアプリの『合計消費カロリー』画面専用(あすけんからは取らない)",
...
def build_extraction_prompt():
    ...
    return (
        ...
        "【カロリー2項目の取得元の固定ルール(最重要・例外なし)】\n"
        "intakeCalories(摂取カロリー)と calories(消費カロリー)は、それぞれ決まった画面からしか"
        "取ってはいけない。取得元が違えば、たとえ数値が読めても必ず null にすること。\n\n"
        "■ intakeCalories は『あすけん』の摂取カロリー画面からのみ取る\n"
        "あすけんの画面は、上部に『摂取カロリー』『消費カロリー』『PFCバランス』の3つのタブが横に並び、"
        "選択中のタブは文字が濃い緑色で下に緑の下線が引かれている。その下に"
        "『合計 XXXX kcal』(左・オレンジ色)と『目標 XXXX kcal』(右・緑色)が並ぶ。\n"
        "- 選択中のタブが『摂取カロリー』のときだけ、『合計』の数値を intakeCalories に入れる。\n"
        "- 『目標 XXXX kcal』は目標値であって実績ではないので必ず無視する。\n"
        "- 『朝食 XXXkcal』のような食事ごとの小計は合計ではないので入れない。\n"
        "- 選択中のタブが『摂取カロリー』だと確認できない場合は intakeCalories を null にする。\n\n"
        "■ calories は『ヘルスケアアプリの合計消費カロリー画面』からのみ取る\n"
        "この画面は、上部に『合計消費カロリー』という見出しがあり、『日/週/月/3か月/年』の期間タブと"
        "日付が表示され、大きな数字で『3,498/2,770 kcal』のように"
        "『実績 / 目標 kcal』がスラッシュ区切りで並び、下に時間帯別の棒グラフが出ている。\n"
        "- スラッシュの左側(実績)の数値を calories に入れる。右側は目標なので必ず無視する。\n"
        "- この『合計消費カロリー』画面でない限り、calories は必ず null にする。\n\n"
        "■ あすけんからは消費カロリーを絶対に取らない\n"
        "あすけんに『消費カロリー』タブが存在し、そこに数値が表示されていても、"
        "calories には絶対に入れず null のままにすること。消費カロリーの取得元は"
        "ヘルスケアアプリの『合計消費カロリー』画面のみと決まっている。\n\n"
        ...
        "6. intakeCalories は『あすけんの摂取カロリー画面』以外からは絶対に取らない。"
        "calories は『ヘルスケアアプリの合計消費カロリー画面』以外からは絶対に取らない。"
        "取得元が確認できない数値は、読めたとしても null にする。\n"
        "7. 出力はJSONオブジェクトのみ。前置き、説明文、マークダウンのコードフェンス(```)は"
        "一切含めない。\n\n"
